...
public class CWConversation(SSOracleBehavior owner, SSOracleBehavior.ConversationBehavior convBehav, string convo, DialogBox dialogBox) : SSOracleBehavior.PebblesConversation(owner, convBehav, ID.None, dialogBox)
{
    ...
    {
        var res = type == CreatureTemplate.Type.Vulture || type == CreatureTemplate.Type.KingVulture || type == CreatureTemplate.Type.BigEel || type == CreatureTemplate.Type.MirosBird || (ModManager.DLCShared && (type == DLCSharedEnums.CreatureTemplateType.MirosVulture || type == DLCSharedEnums.CreatureTemplateType.TerrorLongLegs)) || type == CreatureTemplate.Type.RedCentipede || type == CreatureTemplate.Type.Deer || type == CreatureTemplate.Type.TempleGuard || type == CreatureTemplate.Type.DaddyLongLegs || type == CreatureTemplate.Type.BrotherLongLegs;
